...
const photoSchema = require('../modeles/photo.modele')
...
exports.checkTokenGetProfile = (req, res) => {
    ...
    if(!err){
      ...
      const userreq = await userSchema.findOne({_id: req.params.id}) ;
        if(userreq.username != userToken){
          ...
          res.render('profile/profile', {user:userreq , file : file})}
        else{  
          const user = await userSchema.findOne({username:userToken.username}); 
          res.cookie("auth", token);
          res.render('profile/profile', {user:user , file: file})}}
    else{ 
...
exports.checkTokenGetUpload = (req, res) => {
  ...
  if(!err){
    ...
    const userreq = await userSchema.findOne({_id: req.params.id}) ;
      if(userreq.username != userToken){
        ...
        res.cookie("auth", newToken);
        res.render('profile/upload', {user:userreq , file : file})}
      else{  
        const user = await userSchema.findOne({username:userToken.username}); 
        res.cookie("auth", token);
        res.render('profile/upload', {user:user , file: file})}}
  else{ 
    res.render('logg/signin') 
  }
}); 
...
exports.checkTokenGetPhoto = (req, res) => {
 

            photoSchema.find({}, (items) => {
            console.log(items);    
            res.render('profile/download', {items : items})})
